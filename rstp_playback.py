...
def create_output_directory(output_file: str, ip_tag: str):
    ...
    output_dir = Path(f'/mnt/storage/cctv/66{ip_tag[-3:]}/{date_subfolder}/videos/')
...
def process_raw_video(input_file, output_file):
    ...
    ffmpeg_command = f"ffmpeg -i {input_file} -c:v libx264 -crf 28 -preset slow -an -vf \"scale=1280:1280\" -r 30 {output_file}"
...
def main(ip, is_fisheye, port=0):
    ...
    for i in range(len(clips)):
        ...
        for attempt in range(1, MAX_DOWNLOAD_RETRIES + 1):
            ...
            if is_fisheye:
                filename = f"{clock}_raw.mp4".replace("-", '_')
                output_file = out_dir / filename
                p_status = download_video_(rtsp_url, output_file)
                if p_status == 0:
